      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {steps.map((step, index) => (
            <motion.div
              key={index}
              className="relative group flex justify-center" // centered
              variants={itemVariants}
              whileHover={{ scale: 1.03, transition: { duration: 0.2 } }}
            >
              <Card className={`relative overflow-hidden bg-gradient-to-br ${step.bgGradient} border-2 ${step.borderColor} shadow-lg hover:shadow-2xl transition-all duration-300 h-full max-w-md`}>
